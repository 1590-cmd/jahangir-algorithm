...
CGSBRMD_3diffsize<-function(v,p,i,D,sp2,sp3){
  ...
  if(any(p<=1)!=0) stop("p=Period sizes: Each period size must be greater than 1")
  ...
  setMethod("show", "stat_test", function(object) {
    ...
    cat(row, "\n")
    cat("Following are required sets of shifts to obtain the minimal classes of CBRMDs for
", "v=" ,object$R[1], ",","p1=",object$R[2],"p2=",object$R[3], "and", "p3=",object$R[4],"\n")
    row <- paste(rep("=", 52), collapse = "")
    cat(row, "\n")
    print(object$S[[1]])
    cat("\n")
...
(H<-CGSBRMD_3diffsize(p=c(9,7,5),v=30,i=1,D=16,sp2=2,sp3=2))
H$G
