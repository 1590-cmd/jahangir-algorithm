...
#
# Coded by jahngir et al., 2025-2026 
# Version 2.1.0  (2025-07-20)
#################################################################################
...
CGSBRMD_2diffsize<-function(v,p,i,D=1,sp2=1){
  ...
  setMethod("show", "stat_test", function(object) {
    ...
    cat(row, "\n")
    cat("Following are required sets of shifts to obtain the minimal classes of 
CBRMDs", "v=" ,object$R[1], ",","p1=",object$R[2],"and","p2=",object$R[3],"\n")
    row <- paste(rep("=", 51), collapse = "")
    cat(row, "\n")
    print(object$S[[1]])
    cat("\n")
  ...
  if(sp2==1){
    ...
    if(v%%2==0){
      if(D==9){ 
        v=p[1]*i+p[2]+3
        A<-c(2,3:((v-4)/2),(v/2),((v+2)/2),((v+4)/2):(v-1))
        A1<-c(grouping2(A,p,v,i,sp2))
        A2<-c(v,p);names(A2)<-c("V","p1","p2")
        x<-list(S=A1$B1,G=A1$B3,R=A2,A=A)
      }
      if(D==10){
        v=p[1]*i+p[2]+2
        A<-c(0,2,3:((v-4)/2),(v/2),((v+2)/2),((v+4)/2):(v-1))
        A1<-c(grouping2(A,p,v,i,sp2))
        A2<-c(v,p);names(A2)<-c("V","p1","p2")
        x<-list(S=A1$B1,G=A1$B3,R=A2,A=A)
      }
...
(H<-CGSBRMD_2diffsize(p,i,v,D,sp2))
(H<-CGSBRMD_2diffsize(p=c(5,4),v=20,i=3,D=16,sp2=2))
H$G
(H<-CGSBRMD_2diffsize(p=c(5,3),v=11,i=3,D=4,sp2=2))
(design_CGSBRMD(H))
