...
# D=9: Minimal CWBRMDs-II
# D=10: Minimal CSBGRMDs-II 
#   P: Period size
...
CGSBRMD_equalsize<-function(v,p,i,D=1){
  ...
  setMethod("show", "stat_test", function(object) {
    ...
    cat(row, "\n")
    cat("Following are required sets of shifts to obtain the classes of minimal 
CBRMDs", "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    row <- paste(rep("=", 51), collapse = "")
    cat(row, "\n")
    print(object$S[[1]])
  })
  ...
  if(v%%2==0){
    ...
    if(D==14){  
      v=p*i+3
      A<-c(0,1,2:((v-4)/2),((v+4)/2),((v+4)/2):(v-1))
      A1<-grouping1(A,p,v,i)
      A2<-c(v,p);names(A2)<-c("V","P")
      x<-list(S=A1$B1,G=A1$B2,R=A2,A=A)
    }
...
# example#1
(H<-CGSBRMD_equalsize(v=12,p=5,i=3,D=16))
H$G
(D<-design_CGSBRMD(H))
